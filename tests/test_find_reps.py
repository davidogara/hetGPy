...
def test_find_reps_on_mcycle():
    ...
    model = hetGP.hetGP()
    mcycle = pd.read_csv('tests/mcycle.csv',index_col=False)
    ...
    X0 = pd.read_csv('tests/X0_no_normal_no_rescale.csv')['V1'].values
    Z0 = pd.read_csv('tests/Z0_no_normal_no_rescale.csv')['.'].values
    Z  = pd.read_csv('tests/Z_no_normal_no_rescale.csv')['.'].values
    mult = pd.read_csv('tests/mult_no_normal_no_rescale.csv')['.'].values
    ...
    Zlist = pd.read_csv('tests/Zlist_no_normal_no_rescale.csv')['.'].str.split('|').values
