...
def crit_qEI(x, model, cst=None, preds=None):
    ...
    if len(x.shape) == 1:
        x = x.reshape(-1, 1)
    if preds is None or preds.get("cov") is None:
...
def crit_search(model, crit, replicate=False, Xcand=None, control=dict(tol_dist=1e-6, tol_diff=1e-6, multi_start=20, maxit=100, maximin=True, Xstart=None), seed=None, ncores=1, **kwargs):
    crit_to_function = {"crit_EI": crit_EI, "crit_cSUR": crit_cSUR, "crit_ICU": crit_ICU, "crit_MCU": crit_MCU, "crit_MEE": crit_MEE, "crit": crit_tMSE}
    ...
    if replicate:
        ...
        for i in range(model.X0.shape[0]):
            res.append(crit_func(x=model.X0[i : i + 1, :], model=model, **kwargs))
        return dict(par=model.X0[np.argmax(res), :].reshape(1, -1), value=np.max(res), new=False, id=np.argmax(res))
    if control is None:
    ...
    if control.get("tol_dist") is None:
        control["tol_dist"] = 1e-6
    if control.get("tol_diff") is None:
        control["tol_diff"] = 1e-6
    ...
    if Xcand is None:
        ...
        crit_to_args = {
            ...
            "crit_ICU": {"model": model, "thres": kwargs.get("thres", 0), "Xref": kwargs.get("Xref", None), "w": kwargs.get("w", None), "preds": kwargs.get("preds", None), "kxprime": kwargs.get("kxprime", None)},
            "crit_tMSE": {"model": model, "thres": kwargs.get("thres", 0), "preds": kwargs.get("preds"), "seps": kwargs.get("seps", 0.05)},
            "crit_MCU": {"model": model, "thres": kwargs.get("thres", 0), "gamma": kwargs.get("gamma", 2), "preds": kwargs.get("preds", None)},
        }

        def local_opt_fun(i):
            out = minimize(
                x0=Xstart[i, :], fun=fn, jac=gr, args=tuple(crit_to_args[crit].values()), method="L-BFGS-B", bounds=[(0, 1) for _ in range(d)], options=dict(maxiter=control["maxit"], ftol=control.get("factr", 10e9) * np.finfo(float).eps, gtol=control.get("pgtol", 0))  # ,  # ,  # should map to pgtol
            )
            python_kws_2_R_kws = {"x": "par", "fun": "value", "nfev": "counts"}
        ...
        par[par < 0] = 0
        par[par > 1] = 1
        res = dict(par=par.reshape(1, -1), value=crit_func(par, **crit_to_args[crit]), new=True, id=None)
        ...
        if control["tol_dist"] > 0 and control["tol_diff"] > 0:
            ...
            if np.min(dists) < control["tol_dist"]:
                ...
                res = dict(par=model.X0[[argmin], :], value=crit_func(x=model.X0[argmin, :], **crit_to_args[crit]), new=False, id=argmin)
            else:
                ...
                if (res["value"] - crit_rep) / res["value"] < control["tol_diff"]:
                    res = dict(par=model.X0[[id_closest], :], value=crit_rep, new=False, id=id_closest)
        else:
            ## Discrete
            inputs = []
            for i in range(Xcand.shape[0]):
                ...
                kw[i] = i
                inputs.append(kw)
            res = Parallel(n_jobs=ncores)(delayed(fn)(**kw) for kw in inputs)
...
def crit_optim(model, crit, h=2, Xcand=None, control=dict(multi_start=10, maxit=100), seed=None, ncores=1, **kwargs):
    """
    crit_optim
    """
    d = model.X0.shape[1]
    if crit == "crit_IMSPE":
        raise ValueError("crit_IMSPE is intended to be optimized by IMSPE_optim")
    ...
    for i in range(1, h + 1):
        ...
        crit_A = crit_search(model=newmodelA, crit=crit, replicate=True, control=control, seed=seed, ncores=ncores, **kwargs)
    ...
    if h == 0:
        ...
        if Xcand is None:
            crit_C = crit_search(model=newmodelB, crit=crit, control=dict(Xstart=crit_B["par"], maxit=control["maxit"], tol_dist=control["tol_dist"], tol_diff=control["tol_diff"]), ncores=ncores, **kwargs)
        else:
            crit_C = crit_B.copy()
        ...
        for i in range(h):
            ...
            if i < h:
                ...
                for j in range(i, h):
                    ...
                    crit_C = crit_search(model=newmodelC, crit=crit, replicate=True, control=control, ncores=ncores, **kwargs)
                    path_C.append(crit_C)
            if crit_C["value"] < crit_A["value"]:
                return dict(par=new_designB, value=crit_C["value"], path=path_B + path_C)
